...
func TestResolve_preparationNoMutationOnError(t *testing.T) {
	...
	reg := res.New()
	if err := reg.AddWithTags(defaultAPI, res.TagReplaceable); err != nil {
		t.Fatal(err)
	}
	if err := reg.Add(userAPI); err != nil {
		t.Fatal(err)
	}
	if err := reg.Add(repo1); err != nil {
		t.Fatal(err)
	}
	if err := reg.Add(repo2); err != nil {
		t.Fatal(err)
	}
	if err := reg.Add(repo3); err != nil {
		t.Fatal(err)
	}
	if err := reg.Add(&mainSrv{}); err != nil {
		t.Fatal(err)
	}
	if err := reg.Add(&mockService{}); err != nil {
		t.Fatal(err)
	}

	err := Resolve(reg)
	if !errors.Is(err, ErrTooManyImplementations) {
		t.Fatalf("expected too many implementations, got %v", err)
	}
	if len(reg.GetByType(reflect.TypeFor[*apiHandler]())) != 2 {
		t.Fatal("expected concrete cleanup not applied when interface validation fails")
	}
	if len(reg.GetByInterface(reflect.TypeFor[mockRepo]())) != 3 {
		t.Fatal("expected interface pool unchanged on preparation error")
	}
}
